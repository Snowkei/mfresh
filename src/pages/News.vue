  ...
  <div>
    ...
    <div class="main container">
      <div class="news">
        <ul>
          ...
          <li v-for="(item,index) in news.data" :key="index">
            ...
            <router-link :to="{path:'/news_details', query:{nid:item.nid}}">{{item.title}}</router-link>
            <!-- <a href>{{item.title}}</a> -->
          </li>
      ...
      <div class="pages">
        ...
        <a v-show="news.pageNum!=1" class="prev" @click.prevent="getNews(news.pageNum-1)">上一页</a>

        <a v-show="news.pageNum==1" @click.prevent class="default">上一页</a>

        <a
          v-for="n in news.pageCount"
          :key="n"
          :class="n==news.pageNum ? 'cur':''"
          @click.prevent="getNews(n)"
        >{{n}}</a>
        <!-- <a href="">2</a> -->
