    ...
    <div class="login_box">
      <div class="container">
        <div class="login">
          ...
          <button @click.prevent="login()" type="button" id="login">立 即 登 录</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    login() {
      ...
      var params = "unameOrPhone=" + this.uname + "&upwd=" + this.upwd;
      ...
      this.$axios
        ...
        .then(res => {
          console.log(res);
          //判断 code==1 就是登录成功, 否则是失败
          //如果成功, 则提示登录成功, 保存信息到session, 跳转到首页
          if (1 == res.data.code) {
            alert("登录成功!");
