  ...
  <div>
    ...
    <div class="main container">
      <div class="news_details">
        <h2>{{detail.title}}</h2>
        <span>发布时间：{{detail.pubTime|dateFormat}}</span>
        <!-- v-html 绑定的内容 接收html解析 -->
...
export default {
  ...
  // 挂载时查看当前路由情况
  mounted() {
    // 挂载只是个声明周期中的一个环节, 在这个环节中调用方法
    this.getDetail();
  },
  // 设计的规范: 函数应该写在 methods 中
  methods: {
    getDetail() {
      ...
      /**
       * 自己尝试完成: 发送axios post 请求
       * url: news_detail.php
       * 参数: nid=xxx
       *
       * 请求成功后, 更新拿到的数据到页面上, 显示内容
       *
       */
    }
  },
